...
SERVICE_NAME="${SERVICE_NAME:-vyla-stage-admin}"
ECR_REPOSITORY_NAME="${ECR_REPOSITORY_NAME:-vyla-stage-admin}"
NEXT_PUBLIC_API_URL="${NEXT_PUBLIC_API_URL:-https://stage.api.vyla.health}"
...
if [[ "$ACCOUNT_ID" != "$EXPECTED_ACCOUNT_ID" ]]; then
  echo "refusing stage admin deploy: authenticated AWS account ${ACCOUNT_ID} does not match expected ${EXPECTED_ACCOUNT_ID}"
  exit 1
fi
...
print(json.dumps({
    ...
    "Statement": [
        {
            "Sid": "AllowLightsailStageAdminPull",
            "Effect": "Allow",
...
deploy_container_service() {
  ...
  deployment_json="$TMP_DIR/stage-admin-deployment.json"
...
print_summary() {
  ...
  echo "Stage admin deploy complete."
